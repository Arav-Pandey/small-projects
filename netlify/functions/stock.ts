...
export const handler: Handler = async (event) => {
  try {
    ...
    if (!event.body)
      return {
        statusCode: 401,
        body: JSON.stringify({ error: "No information message. Sorry bye" }),
      };
    ...
    return {
      ...
      body: JSON.stringify({
        ...
        data,
        stockQuoteRequestLimit: user?.stockQuoteRequestLimit ?? 100,
        stockQuoteRequestsUsed: user?.stockQuoteRequestsUsed,
      }),
    };
  } catch (error) {
